...
class JiraIntegrationEndpointTests(unittest.TestCase):
    ...
    def test_jira_import_logs_usage_and_persists_requirement_mappings(self) -> None:
        ...
        with patch("app.routers.integrations_jira.enforce_billing_access", return_value=object()):
            with patch("app.routers.integrations_jira.start_workflow_run", return_value="run-jira-import-1") as start_run:
    ...
    def test_jira_connection_upsert_returns_saved_summary(self) -> None:
        ...
        with patch("app.routers.integrations_jira.start_workflow_run", return_value="run-jira-connection-1"):
            with patch("app.routers.integrations_jira.complete_workflow_run"):
                with patch("app.routers.integrations_jira.upsert_jira_connection", return_value=service_response):
                    with TestClient(app) as client:
                        response = client.post(
    ...
    def test_jira_project_issue_types_returns_project_issue_types(self) -> None:
        ...
        with patch("app.routers.integrations_jira.list_jira_project_issue_types", return_value=service_response):
            with TestClient(app) as client:
    ...
    def test_jira_sync_preview_returns_preview_summary(self) -> None:
        ...
        with patch("app.routers.integrations_jira.start_workflow_run", return_value="run-jira-preview-1") as start_run:
            with patch("app.routers.integrations_jira.complete_workflow_run") as complete_run:
                with patch("app.routers.integrations_jira.record_usage_event", return_value="event-jira-preview-1") as record_event:
                    with patch("app.routers.integrations_jira.preview_jira_requirement_sync", return_value=preview_response) as preview_service:
                        with TestClient(app) as client:
                            response = client.post(
