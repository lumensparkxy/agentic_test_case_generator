...
class AutomationEndpointTests(unittest.TestCase):
    ...
    def test_execution_preview_logs_usage_and_returns_buckets(self) -> None:
        ...
        with patch("app.routers.automation.start_workflow_run", return_value="run-execution-preview-1") as start_run:
            with patch("app.routers.automation.complete_workflow_run") as complete_run:
                with patch("app.routers.automation.record_usage_event", return_value="event-execution-preview-1") as record_event:
                    with patch("app.routers.automation.preview_execution", return_value=service_response) as preview:
                        with TestClient(app) as client:
    ...
    def test_execution_run_logs_usage_and_returns_summary(self) -> None:
        ...
        with patch("app.routers.automation.start_workflow_run", return_value="run-execution-run-1") as start_run:
            with patch("app.routers.automation.complete_workflow_run") as complete_run:
                with patch("app.routers.automation.record_usage_event", return_value="event-execution-run-1") as record_event:
                    with patch("app.routers.automation.run_execution", return_value=service_response) as run:
                        with TestClient(app) as client:
