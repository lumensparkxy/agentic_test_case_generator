...
class AuthEndpointTests(unittest.TestCase):
    ...
    def test_google_login_returns_access_token_and_user(self) -> None:
        user = AuthUser(
            sub="google-user-1",
        ...
        with patch("app.routers.auth.get_auth_settings", return_value=SimpleNamespace(google_client_ids=["web-client-id"])):
