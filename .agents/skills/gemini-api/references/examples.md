...
Consult only the requested operation/language. These upstream tutorial examples
are not repository defaults or authorization to change infrastructure. Derive
project, region, identity, model, SDK, and runtime settings from the current
repository configuration; verify version-sensitive details before using them.
...
Use the model already selected in repository configuration or explicitly requested
by the user. Model names in the samples below are historical examples, not defaults.
Verify support for the required capability before proposing a model change.
...
- **Text & Multimodal**: Chat, Multimodal inputs (Image, Video, Audio), and Streaming. See [references/text_and_multimodal.md](text_and_multimodal.md)
- **Embeddings**: Generate text embeddings for semantic search. See [references/embeddings.md](embeddings.md)
- **Structured Output & Tools**: JSON generation, Function Calling, Search Grounding, and Code Execution. See [references/structured_and_tools.md](structured_and_tools.md)
- **Media Generation**: Image generation, Image editing, and Video generation. See [references/media_generation.md](media_generation.md)
- **Bounding Box Detection**: Object detection and localization within images and video. See [references/bounding_box.md](bounding_box.md)
- **Live API**: Real-time bidirectional streaming for voice, vision, and text. See [references/live_api.md](live_api.md)
- **Advanced Features**: Content Caching, Batch Prediction, and Thinking/Reasoning. See [references/advanced_features.md](advanced_features.md)
- **Safety**: Adjusting Responsible AI filters and thresholds. See [references/safety.md](safety.md)
- **Model Tuning**: Supervised Fine-Tuning and Preference Tuning. See [references/model_tuning.md](model_tuning.md)
