...
const requirementAnalysis = [
	{
		...
		business_rules: [
			{ id: "REQ-001-BR-01", requirement_id: "REQ-001", title: "Account holdings are visible", description: requirements[0].text, rule_type: "Business" },
		],
		field_constraints: [],
		role_permissions: [
			{ id: "REQ-001-RP-01", requirement_id: "REQ-001", role: "Portfolio Manager", action: "View holdings and exposure", effect: "Allow", conditions: "Authenticated user assigned to account." },
		],
		state_transitions: [],
		risk_signals: [
			{ id: "REQ-001-RS-01", requirement_id: "REQ-001", title: "Incorrect exposure displayed", rationale: "Could lead to inappropriate trading decisions.", category: "Data Integrity", severity: "High" },
		],
	...
	},
	{
		...
		business_rules: [
			{ id: "REQ-002-BR-01", requirement_id: "REQ-002", title: "Mandate breaches block approval", description: requirements[1].text, rule_type: "Authorization" },
		],
		field_constraints: [
			{ id: "REQ-002-FC-01", requirement_id: "REQ-002", field_name: "requested trade", description: "Trade must remain within mandate limits.", constraint_type: "Range", operator: "<=", value: "configured mandate limit", negative_example: "Trade exceeds mandate limit." },
		],
		role_permissions: [
			{ id: "REQ-002-RP-01", requirement_id: "REQ-002", role: "Portfolio Manager", action: "Approve blocked trade", effect: "Deny", conditions: "Mandate or restricted-list breach exists." },
		],
		state_transitions: [
			{ id: "REQ-002-ST-01", requirement_id: "REQ-002", entity: "Trade", from_state: "Submitted", to_state: "Blocked", trigger: "Compliance rule breach", guards: "Restricted-list or mandate check fails." },
		],
		risk_signals: [
			{ id: "REQ-002-RS-01", requirement_id: "REQ-002", title: "Restricted-list breach", rationale: "Regulatory and client mandate impact.", category: "Compliance", severity: "Critical" },
		],
...
const testCases = [
	{
		...
		steps: [
			{ step: 1, action: "Sign in as demo.manager@example.test and open Portfolio Overview.", expected: "Portfolio Overview loads with the account selector visible.", test_data: "ACC-1001" },
			{ step: 2, action: "Select ACC-1001 from the account selector.", expected: "Holdings, cash, exposure, and exception panels refresh for ACC-1001.", test_data: "ACC-1001" },
		],
	...
	},
	{
		...
		steps: [
			{ step: 1, action: "Open submitted trade TRD-2002 as a portfolio manager.", expected: "Trade details show status Submitted and mandate warning is visible.", test_data: "TRD-2002" },
			{ step: 2, action: "Click Approve trade.", expected: "Approval is blocked with a mandate breach message.", test_data: null },
			{ step: 3, action: "Review workflow status and audit event.", expected: "Trade status changes to Blocked and an audit record is created.", test_data: "Audit event type TRADE_APPROVAL_BLOCKED" },
		],
...
async function installRoutes(page) {
	...
	await page.route("**/requirements/parse", async (route) => jsonResponse(route, {
		...
		iteration_history: [
			{ iteration: 1, actor: "RequirementReviewerAgent", approved: true, score: 94, threshold: 85, summary: "Approved with one ownership follow-up.", artifact_count: 2, artifact_ids: ["REQ-001", "REQ-002"], blocking_issues: [], suggestions: [] },
		],
	...
	await page.route("**/testcases/generate", async (route) => jsonResponse(route, {
		...
		iteration_history: [
			{ iteration: 1, actor: "TestCaseValidatorAgent", approved: true, score: 96, threshold: 90, summary: "Coverage gates met.", artifact_count: 2, artifact_ids: ["TC-001", "TC-002"], blocking_issues: [], suggestions: [] },
		],
		...
		workflow_diagnostics: { status: "completed", warnings: [], parser_failures: [], used_fallback: false, attempt_count: 1, best_iteration: 1 },
	}));
	await page.route("**/automation/execution/preview", async (route) => jsonResponse(route, {
	...
	await page.route("**/export/json", async (route) => jsonResponse(
		route,
		{ export_format: "test_cases_v1", total_count: testCases.length, test_cases: testCases },
		200,
...
async function main() {
	...
	await page.addInitScript(({ storageTokenKey, storageUserKey, authToken, authUser }) => {
