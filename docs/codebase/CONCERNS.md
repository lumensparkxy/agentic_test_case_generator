...
|----------|---------|----------|--------|------------------|
| High | Large, high-churn orchestration files remain central to the app | `frontend/src/App.jsx`, `backend/app/agents/test_case_agent.py`, recent git history | Higher regression and merge risk for UI workflow and prompt orchestration | Continue behavior-preserving extraction under issue-scoped frontend and agent refactor tasks |
...
| Contract facade compatibility | Pydantic contracts now live in domain modules, but existing callers still import through the legacy facade | `backend/app/contracts/`, `backend/app/models.py` | New contract changes could drift if maintainers bypass the closest domain module or forget schema checks | Add new contracts to the relevant domain module and keep running OpenAPI export plus generated frontend API checks |
...
| `backend/app/contracts/` and `backend/app/models.py` facade | Shared API contracts are split by domain but still define the public API surface | Contract changes affect API consumers | Treat changes as contract changes; run OpenAPI export and generated frontend API checks |
